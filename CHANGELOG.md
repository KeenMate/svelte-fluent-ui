...
- **Docs pages standardized — batch 1** - Autocomplete, Combobox, Select, and Grid demo pages migrated to the shared Calendar-reference layout: outer `<Stack>`, `<h1>`, description, References `<Card>`, 3-column API `<Grid>` (Properties / Callbacks / Slots using `<h2>` inside Cards), then Examples wrapped in a single `<Card>` with `<h3>` subsections. Select and Grid have a secondary entity (Option / GridItem) rendered in an extra Card or second API Grid below the primary one. Autocomplete's callbacks/slots were normalized to the shared Property shape (name/type/default/description) so all three API QuickGrids share the same columns, and duplicate rows where the same entry appeared in both Properties and its native Callbacks/Slots card were removed
