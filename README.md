...
## What's New in v1.5.0-rc04

- **Grid — container queries — size columns to a container, not the viewport** — `Grid`/`GridItem` breakpoints (`xs`/`sm`/`md`…) were viewport-only, so a grid reused inside a sidebar, panel, or splitter couldn't reflow to its own width. New opt-in `container` prop marks the Grid as a CSS containment context and switches its children to `@container` sizing; `<Grid container>` now reflows on the Grid's box width. `containerId="name"` instead binds sizing to a named ancestor container. Column values stay on `data-*` attributes (no inline styles), so it's clean under a strict CSP `style-src`, and the container rules are additive — viewport-mode grids are untouched.
- **Grid — solution-wide breakpoint config via CSS variables** — Container-query thresholds can't read `var()` directly, so a new `configureGridBreakpoints()` bridges it: call it once (it reads `--fluent-grid-breakpoint-*` from `:root`, or takes an explicit object) to set breakpoints app-wide, matching the library's `--fluent-*` theming convention. Per-Grid `containerBreakpoints={{sm:300, md:450}}` overrides a single Grid; resolution is per-Grid → global → built-in defaults. Rules are injected via a constructed stylesheet, so named containers and custom breakpoints stay CSP-safe (no `'unsafe-inline'` or nonce needed). New exports: `configureGridBreakpoints`, `getEffectiveBreakpoints`, `DEFAULT_GRID_BREAKPOINTS`, `GridBreakpoints`.
