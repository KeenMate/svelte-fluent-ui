...
- **`QuickGrid` per-row-type editability** — `column.isEditable` (renamed from `column.editable`) now accepts a `(row) => boolean` callback so heterogeneous trees can express "only some row types are editable" without the consumer hand-rolling click handlers. Two shapes: per-column (`isEditable: (row) => row.kind === "employee"` — different columns can be editable for different row types) and grid-level (`isRowEditable: (row) => boolean` — single predicate gating the whole row). Three-stage gate: master `editable` → `isRowEditable` → `column.isEditable`. Navigate-mode Tab traversal is row-aware so the cursor skips read-only cells correctly. New demo at `/components/quickgrid-tree` ("Editable per row type — teams vs employees")
- **`QuickGrid` `column.nowrap` prop** — body cells never wrap. Combine with `maxWidth` for ellipsis truncation, with `autoWidth` to size the column to `max(header, longest cell)`, or use alone for short identifiers like SKU/code columns. Demo on `/components/quickgrid` ("Nowrap columns & ellipsis truncation") shows all three modes
- **BREAKING — `QuickGrid` column property `editable` renamed to `isEditable`** — the boolean form is unchanged; the new function form is additive. Migration: rename `editable: true` → `isEditable: true` on every column literal. Grid-level `editable` prop is unchanged

### Also in v1.0.0-rc16

- **`QuickGrid` component-prefixed CSS variables** — new `--fluent-quickgrid-header-bg`, `--fluent-quickgrid-header-hover-bg`, `--fluent-quickgrid-header-sorted-bg`, `--fluent-quickgrid-stripe-bg`, `--fluent-quickgrid-row-hover-bg` tokens decouple QuickGrid surfaces from the generic `--neutral-layer-*` palette. Themes that overrode `--neutral-layer-2` for navbar branding (e.g. DHL yellow) used to drag QuickGrid header + striped rows along at full saturation; consumers can now tint the grid independently. Two-layer fallbacks keep existing themes rendering identically
- **Site settings store applies to the document** — the `settings` store now pushes every change to `document.documentElement` (`data-theme`, `dir`, `--accent-fill-*`, `--foreground-on-accent-*`, `--neutral-layer-1`, mirrored `--fluent-*` tokens). Theme mode `'system'` resolves via `matchMedia` and re-applies on OS-level theme changes. Foreground-on-accent is computed via W3C luminance threshold so dark accents get white labels and light accents (PowerBI yellow) get black labels
- **`Tabs` active label and `QuickGrid` sort triangle now track accent** — both surfaces use `var(--accent-fill-rest, currentColor)` so theme switches propagate to indicator + active label visually
